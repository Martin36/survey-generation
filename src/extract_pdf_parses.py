...
RESULTS_PATH = "/ukp-storage-1/funkquist/cs_pdf_parses.json"
# OUTPUT PATHS
SURVEY_PAPERS_PATH = "/ukp-storage-1/funkquist/cs_survey_papers.json"
PDF_PARSE_SURVEY_PAPERS_PATH = "/ukp-storage-1/funkquist/cs_pdf_parse_survey_papers.json"

NUM_PROCESSES = 8

with open(SURVEY_PAPERS_PATH) as f:
    survey_papers = json.loads(f.read())
paper_ids = [p["paper_id"] for p in survey_papers]
...
def explore_pdf_parses(file_path: str):
    ...
    with gzip.open(file_path, "rb") as file:
        ...
        for line in reader.readlines():
            ...
            if pdf_parse["paper_id"] not in paper_ids:
                continue

            survey_papers.append(pdf_parse)
    ...
    return stat_counter, time_counter, survey_papers
...
if __name__ == "__main__":
    ...
    all_survey_papers = list()

    with multiprocessing.Pool(processes=NUM_PROCESSES) as p:
        for stat_count, time_count, survey_papers in tqdm.tqdm(p.imap(explore_pdf_parses, pdf_parse_file_paths),
            ...
            all_survey_papers += survey_papers
    ...
    with open(PDF_PARSE_SURVEY_PAPERS_PATH, "w", encoding="utf-8") as file:
        json.dump(all_survey_papers, file, indent=2)
